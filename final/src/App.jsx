...
function App() {
	...
	// Shows loading indicator for VIEWS
	function loadingPage() {
		dispatch({type: ACTIONS.LOADING_PAGE, isPageLoading: true});
		setTimeout(() => {
			dispatch({type: ACTIONS.LOADING_PAGE, isPageLoading: false});
		}, LOADING_DELAY);
	}
	...
	function onClickCart() {
		...
		loadingPage();//Show loading indicator...
	...
	function onCheckOut() {
		dispatch({type: ACTIONS.START_CHECKING_OUT}); // Show loading indicator to user
		setTimeout(() => {
			dispatch({type: ACTIONS.AFTER_CHECK_OUT}); // Hide the indicator
		}, LOADING_DELAY);
	...
	function checkSession() {
		setTimeout(() => {
			// Setup 2 seconds delay to mock loading resources
			// from the server to the client via Internet
